...
const subject = ref("Email with Inline Image");
...
async function sendEmail() {
  ...
  try {
    const data = await $fetch("/api/send-cid", {
      method: "POST",
      body: { to: to.value, subject: subject.value },
    });
    success.value = `Email with inline image sent successfully!\nEmail ID: ${data.id}`;
  } catch (err: any) {
...
<template>
  <div>
    <PageHeader
      ...
      description="Send an email with inline images using Content-ID (CID) attachments."
    />
    ...
    <div class="code-example">
      <h3>Code Example</h3>
      <pre><code>// server/api/send-cid.post.ts
const placeholderImage = "iVBORw0KGgoAAAA..."; // base64 PNG
  ...
  attachments: [
    {
      ...
      contentId: "logo", // matches "cid:logo" in HTML
    },
  ],
});</code></pre>
    </div>
